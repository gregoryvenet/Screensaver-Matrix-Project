...
class Codeline:
    ...
    def desloc_codeline(self):
        if self.ypos > SCREEN_HEIGHT or len(self.ypos_list) > 40:
            message = " "
            text = font.render(message, True, WHITE)
            screen.fill(BLACK, (self.ypos, self.ypos_list[0], text.get_width() + FONT_SIZE, text.get_height()* self.fill))
...
while True:
    ...
    if current_time - color_time > 10000:  # 10 secondes
        color_change = (color_change + 1) % len(COLORS)
        color_time = pygame.time.get_ticks()

    if current_time - initial_time > 200:  # 0.2 secondes
        generate = True
